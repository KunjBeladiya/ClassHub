...
export const ForumReplyCard = ({ reply, onDelete }) => {
    ...
    const checkLike = async () => {
      try {
        const response = await fetch(
          `${API}/api/v1/forum/reply/checklike?reply_id=${reply.id}`,
          { credentials: "include" }
        );
  ...
  useEffect(() => {
    const fetchLikeCount = async () => {
      try {
        const response = await fetch(
          `${API}/api/v1/forum/reply/like-count?reply_id=${reply.id}`,
          { credentials: "include" }
        );
        const data = await response.json();
  ...
  const handleLike = async () => {
    try {
      if (!isLiked) {
        const response = await fetch(`${API}/api/v1/forum/reply/like`, {
          method: "POST",
        ...
        const response = await fetch(
          `${API}/api/v1/forum/reply/dislike?reply_id=${reply.id}`,
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      const res = await fetch(`${API}/api/v1/forum/delete/reply/${reply.id}`, {
        method: "DELETE",
        credentials: "include",
      });
      ...
      <div className="p-5">
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div
              className="prose max-w-none mb-4 min-h-[100px] p-4 bg-white
            ...
            <div className="flex items-center justify-between mt-4 pt-3 border-t border-gray-100">
              <div className="flex items-center gap-4">
                ...
                >
                  <ThumbsUp size={18} className={isLiked ? "fill-indigo-500" : ""} />
